<template>
    <div class="tbh-app">
        <h3>
            阿里APP
            <a class="more">
                更多
                <i class="el-icon-arrow-right"></i>
            </a>
        </h3>
        <ul class="apps-nav clearfix">
            <li class="nav" v-for="(item, index) in data" :key="index">
                <a>
                    <img :src="item.src" :alt="item.alt" class="app-icon" />
                </a>
                <div class="app-qr">
                    <img
                        class="J_QRCode"
                        :src="`./src/img/col-right/${item.qrCodeSrc}`"
                        :alt="item.qrCodeAlt"
                    />
                    <p>{{item.title}}</p>
                </div>
            </li>
        </ul>
    </div>
</template>
<script>
export default {
    data() {
        return {
            data: [
                {
                    src:
                        "data:image/webp;base64,",
                    alt: "手机app - 淘宝",
                    qrCodeSrc: "app-Code1.jpg",
                    qrCodeAlt: "使用手机扫描淘宝的二维码",
                    title: "扫一扫淘宝"
                },
                {
                    src:
                        "data:image/webp;base64,",
                    alt: "手机app - 天猫",
                    qrCodeSrc: "app-Code2.png",
                    qrCodeAlt: "使用手机扫描天猫的二维码",
                    title: "扫一扫天猫"
                },
                {
                    src:
                        "data:image/webp;base64,",
                    alt: "手机app - 聚划算",
                    qrCodeSrc: "app-Code3.png",
                    qrCodeAlt: "使用手机扫描聚划算的二维码",
                    title: "扫一扫聚划算"
                },
                {
                    src:
                        "data:image/webp;base64,",
                    alt: "手机app - 支付宝",
                    qrCodeSrc: "app-Code4.png",
                    qrCodeAlt: "使用手机扫描支付宝的二维码",
                    title: "扫一扫支付宝"
                },
                {
                    src:
                        "data:image/webp;base64,",
                    alt: "手机app - 旺旺",
                    qrCodeSrc: "app-Code5.png",
                    qrCodeAlt: "使用手机扫描旺旺的二维码",
                    title: "扫一扫旺旺"
                }
            ]
        };
    }
};
</script>
<style lang="less">
.tbh-app {
    font-family: tahoma, arial, "Hiragino Sans GB", "宋体", sans-serif;
    position: relative;
    height: 76px;
    background-color: white;
    h3 {
        font-size: 14px;
        padding-left: 14px;
        height: 30px;
        line-height: 30px;
        position: relative;
        z-index: 2;
        font-weight: 700;
        a {
            position: absolute;
            right: 10px;
            font-size: 12px;
            font-weight: 400;
        }
        a:hover {
            color: #f40;
        }
    }
    .apps-nav {
        padding-left: 23px;
        position: relative;
        height: 44px;
        li {
            transition: all 0s;
            position: relative;
            float: left;
            width: 32px;
            height: 32px;
            margin-right: 21px;
            margin-bottom: 8px;
            a {
                color: #3c3c3c;
                img {
                    width: 32px;
                    height: 32px;
                }
            }
            .app-qr {
                touch-action: none;
                bottom: 39px;
                left: -32px;
                width: 90px;
                height: 102px;
                background: #fff;
                border: 1px solid #dedede;
                text-align: center;
                display: none;
                position: absolute;
                z-index: -1;
                img {
                    width: 66px;
                    height: 66px;
                    padding: 12px;
                }
                p {
                    position: absolute;
                    bottom: 3px;
                    left: 0;
                    right: 0;
                    height: 20px;
                    line-height: 20px;
                    color: #666;
                    overflow: hidden;
                }
            }
        }
        li:hover .app-qr {
            display: block;
            z-index: 2;
        }
    }
}
</style>